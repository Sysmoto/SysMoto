...
class BackupMySQL {
	...
	*/
	...
	private function initFile() {
		...
		$adjustedTimestamp = $currentTimestamp - (4 * 60 * 60);
		$formattedDateTime = date('Y-m-d_H-i-s', $adjustedTimestamp);

		$time = date('Y-m-d_H-i-s', $adjustedTimestamp); //time();
	...
	private function sqlHeader() {
		...
		$currentTimestamp = time();
		$adjustedTimestamp = $currentTimestamp - (4 * 60 * 60);
		$datetime = date('Y-m-d H:i:s', $adjustedTimestamp);
		$this->append(
