...
if(isset($_POST["registrar"])) {
  ...
  if(isset($_POST["item"]) > 0 and $_POST["totalCompra"] > 0) {
    ...
    $id_venta=alta_presupuesto($_POST,$MiConexion,$vendedor);

    $alta_detalle_venta=alta_presupuesto_detalle($_POST["item"],$id_venta,$MiConexion);
    echo "<script> 
            ...
            <?php 
             if(empty($_POST["id_cliente"])) { ?>
              <div class="container-xxl flex-grow-1 container-p-y">
                <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Nuevo Presupuesto</h4>   
...
<script language="javascript">
        ...
        selectArticulo.addEventListener('change', function () {
            ...
            var margen = document.getElementById('margen_gan').value;
            var margen = 1 + (margen / 100);
            var precioVenta = selectedArticulo ? selectedArticulo['ART_PRECIOCOMPRA']  : 0;
            var precioVenta = (precioVenta * margen).toFixed(2); 
            ...
            actualizarTotal(inputCantidad, inputPrecioVenta.value, inputTotal);
             });
            ...
            // Actualizar el total cuando cambie el precio de compra
            actualizarPrecioVenta(margen_gan,inputPrecioCompra,inputPrecioVenta);
            ...
            actualizarTotal(inputCantidad, precioVenta, inputTotal);
        });
    
        cellArticulo.appendChild(selectArticulo);

    }
    function actualizarPrecioVenta(margen_gan,inputPrecioCompra,inputPrecioVenta) {
      var ganancia = parseFloat(margen_gan.value) || 0;
      ganancia = 1 + ( ganancia /100);
      var precioVenta = inputPrecioCompra.value * ganancia;
      precioVenta = precioVenta.toFixed(2);
      inputPrecioVenta.value = precioVenta;
    }

    function actualizarTotal(inputCantidad, precioVenta, inputTotal) {
  
      var total = inputCantidad.value * precioVenta;
...
function calcularSumaParcial() {
    ...
    var multiplicador = 1  + (impuestos / 100) - (descuentos / 100);

    for (var i = 1; i < rowCount; i++) { // Empezar desde 1 para omitir la fila de encabezado
        var inputTotal = tabla.rows[i].cells[6].querySelector('input');
        if (inputTotal) {
